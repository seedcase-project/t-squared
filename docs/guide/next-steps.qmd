...
guide alongside the `TODO.md` to complete those tasks. For example, one of
the first tasks is to start having Git track the project with `git init`.
...
standards and catching errors early. The `justfile` contains a `install-precommit`
recipe that sets up the pre-commit hooks for your project. Run it by opening
a terminal within the project directory and running the following command:
...
For Seedcase Project repositories, we have a custom Quarto extension that
has our theme called `seedcase-theme`. The `justfile` has a recipe called
`update-quarto-theme` that adds this theme to the project. Run it by opening
a terminal within the project directory and running the following command:
...
GitHub repositories for Seedcase Project repositories. While it's
useful for us, it isn't designed or build in a way that makes it
easy for others to use.

For setting up a GitHub repository for the created project that will
be part of the Seedcase Project, you'll need to install our internal
tool [`spaid`](https://github.com/seedcase-project/spaid) and
...
- The workflow `.github/workflows/release-package.yml` requires the
  ...
  GitHub App, which is specific to the Seedcase Project, as well as a creating a
  GitHub secret called `UPDATE_VERSION_TOKEN` and a variable called
...
- The workflow `.github/workflows/add-to-project.yml` requires the
  ...
  GitHub App (another Seedcase app), along with the `ADD_TO_BOARD_TOKEN` secret
  and the `ADD_TO_BOARD_APP_ID` variable of the GitHub App's ID.
...
do this. Before you can host on GitHub Pages, you need to use Quarto
to set things up for you, by running the following command:
...
This is useful if you want to create a DOI for your project, which will
allow you or others to cite it in their work as well as provide more exposure
for your project. Connecting GitHub to Zenodo is very easy, check out
Zenodo's
